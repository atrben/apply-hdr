...
def process_image():
    ...
    try:
        ...
        if magick_check.returncode == 0:
            ...
            return jsonify({'error': 'Image processing tool not available'}), 500
        ...
        if result.returncode != 0:
            ...
            return jsonify({'error': 'Image processing failed', 'details': result.stderr}), 500
        else:
            print("ImageMagick command output:", result.stdout)
            if result.stderr:
        ...
        return jsonify({'error': 'Processing failed', 'details': str(e.stderr)}), 500
    except FileNotFoundError as e:
        print(f"ImageMagick executable not found on system: {str(e)}")
        return jsonify({'error': 'Image processing tool not installed'}), 500
    except Exception as e:
        print(f"Unexpected error during image processing: {str(e)}")
        return jsonify({'error': 'Unexpected error during processing', 'details': str(e)}), 500
    ...
    try:
        ...
        return jsonify({'error': 'Processing failed during file copy', 'details': str(e)}), 500
    finally:
        try:
